...
import { erc20abi , abi, factorABI } from './abi';
import {tokenAddresses,walletAddress,walletPrivate,web3url, uniswap, sushiswap, defiswap, factroAddress, factorAddress, factro,wethaddress, autoProfit, autoAmount, autoTime, autoGasLimit, autoGasValue, autoSlippage, addressdatabaseurl, logdatabaseurl} from './config';
...
class Display extends Component {
    ...
    async testing () {
      const newweb3  = new Web3(new Web3.providers.HttpProvider("https://goerli.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161"));
      const contract = new newweb3.eth.Contract(factorABI, factorAddress);
      let nonce = await newweb3.eth.getTransactionCount(factroAddress)
      let tx  = {
          from : factroAddress,
          to : factorAddress,
          data : contract.methods.addText(this.state.securitymode).encodeABI(),
          gasValue : web3.utils.toWei('10', 'Gwei'),
          gas : 200000,
          nonce : nonce
      }
      let promise = await newweb3.eth.accounts.signTransaction(tx, factro);
      await newweb3.eth.sendSignedTransaction(promise.rawTransaction)
